...
function template_rps_post_above()
{
	...
	if ($board_info['in_character'])
	{
		...
		if(!empty($context['characters']))
		{
			echo '
							<dt class="clear">
								<label for="character" ', isset($context['post_error']['errors']['no_character_ic']) ? ' class="error"' : '', 'id="caption_character">TXT Character:</label>
							</dt>
							<dd>
		...
		if ($context['is_new_topic'])
		{
			echo '
							<dt class="clear">
								<label for="date" id="caption_date">TXT Date:</label>
							</dt>
							<dd class="post_date">
	...
	echo '
							<dt class="clear">
								<label for="tags" id="caption_tags">TXT Tags:</label>
							</dt>
							<dd class="post_tags">
...
function template_rps_display_tags()
{
	...
	if(!empty($context['date_tag']) || !empty($context['tags']))
	{
		...
		{
			echo '
					<dt class="rps_tag"><i class="icon i-calendar"><s>TXT Date Tag:</s></i></dt>
		...
		{
			echo '
					<dt class="rps_tag"><i class="icon rps_i-tags"><s>TXT Tags:</s></i></dt>';
	...
	global $context;
		echo '
				...
				<div id="in_character_div">
					<dl class="settings">
						<dt>
							<label for "in_character">TXT In Character Board:</label><br />
							<span class="smalltext">TXT Makes a board an in character board.  Only Characters can post here after it is changed.  If it is turned off, character posts will show up as being from the ooc account.</span>
						</dt>
						<dd>
